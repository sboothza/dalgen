...
class MySqlAdaptor(Adaptor):
    ...
    def generate_create_script(self, table: Table) -> str:
        ...
        for field in table.fields:
            result.append(f"`{field.name.raw()}` {self.get_field_type(field.type, field.size, field.scale)}"
                          f"{self.get_field_size(field)} {'NOT NULL' if field.required else 'NULL'}"
                          f"{' AUTO_INCREMENT' if field.auto_increment else ''}"
                          f"{' DEFAULT (' + field.default + ')' if field.default else ''}")
        if table.pk:
            result.append(f"PRIMARY KEY ({','.join(table.pk.fields)})")

        for key in table.keys:
            if key.key_type == KeyType.Unique:
                result.append(f"UNIQUE `{key.name.raw()}` ({','.join(key.fields)})")
            elif key.key_type == KeyType.Index:
                result.append(f"INDEX `{key.name.raw()}` ({','.join(key.fields)})")
            elif key.key_type == KeyType.ForeignKey:
                result.append(f"FOREIGN KEY `{key.name.raw()}` ({','.join(key.fields)}) REFERENCES "
                              f"`{key.primary_table}` ({','.join(key.primary_fields)})")

        return f"create table `{table.name.raw()}` ({','.join(result)});"
